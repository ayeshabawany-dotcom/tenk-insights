...
export default function Home() {
  ...
  async function analyze() {
    ...
    try {
      ...
      const { text } = data;
      setMeta(parseMeta(text));
      setSections(parseAnalysis(text));
      setPct(100);
      setPhase("done");
    } catch (e) {
      setErrMsg(e.message);
  ...
  return (
    <>
      ...
      <header style={{ borderBottom:"1px solid rgba(212,175,55,.12)", padding:"16px 28px", display:"flex", alignItems:"center", gap:14, position:"sticky", top:0, zIndex:10, background:"rgba(8,14,26,.95)", backdropFilter:"blur(12px)" }}>
        ...
        {phase === "done" && (
          <button onClick={handleShare} className="abtn"
            style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:7, background: copied ? "rgba(60,184,120,.12)" : "rgba(212,175,55,.1)", border:`1px solid ${copied ? "rgba(60,184,120,.4)" : "rgba(212,175,55,.25)"}`, color: copied ? "#3cb878" : "#d4af37", borderRadius:8, padding:"7px 14px", cursor:"pointer", fontFamily:"sans-serif", fontSize:12, fontWeight:600 }}>
      ...
      <main style={{ maxWidth:760, margin:"0 auto", padding:"40px 20px 100px" }}>
        ...
        <div className={phase === "idle" ? "fadeup d2" : ""} style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(212,175,55,.2)", borderRadius:18, padding:"24px", marginBottom:32, boxShadow:"0 8px 40px rgba(0,0,0,.3)" }}>
          <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
            <input
              ...
              className="inp"
              value={ticker}
              placeholder="Enter any US ticker — AAPL, NVDA, SOUN…"
              disabled={phase === "loading"}
              onChange={e => setTicker(e.target.value.toUpperCase().replace(/[^A-Z]/g,""))}
              onKeyDown={e => e.key === "Enter" && analyze()}
              style={{ flex:1, minWidth:200, background:"rgba(255,255,255,.05)", border:"1px solid rgba(212,175,55,.22)", borderRadius:11, padding:"14px 18px", fontSize:20, fontFamily:"sans-serif", fontWeight:700, color:"#f5ecd5", letterSpacing:"3px" }}
            />
            <button className="abtn" onClick={analyze} disabled={phase==="loading"||!ticker.trim()}
          ...
          <div style={{ marginTop:16, display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
            ...
            {TICKERS.map(t => (
              <button key={t} className="chip" onClick={() => setTicker(t)}
                style={{ background: ticker===t ? "rgba(212,175,55,.22)" : "rgba(212,175,55,.07)", border:`1px solid ${ticker===t ? "rgba(212,175,55,.5)" : "rgba(212,175,55,.18)"}`, color: ticker===t ? "#d4af37" : "#7a6540", borderRadius:7, padding:"4px 11px", fontSize:12, fontFamily:"sans-serif", fontWeight:700, letterSpacing:"1.5px" }}>
                {t}
              </button>
            ))}
          </div>
        ...
        {phase === "loading" && (
          <div style={{ textAlign:"center", padding:"50px 24px" }}>
            <div style={{ fontFamily:"sans-serif", fontSize:12, color:"#3a5868", letterSpacing:"1px", textTransform:"uppercase", marginBottom:28 }}>
              Reading {ticker}&apos;s 10-K annual report…
            </div>
        ...
        {phase === "error" && (
          <div className="fadeup">
            ...
            <button onClick={() => setPhase("idle")} style={{ marginTop:14, background:"transparent", border:"1px solid rgba(212,175,55,.25)", color:"#d4af37", padding:"9px 18px", borderRadius:9, cursor:"pointer", fontFamily:"sans-serif", fontSize:13, fontWeight:600 }}>
              ← Try again
            </button>
          </div>
        )}
        ...
        {phase === "done" && (
          <div>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:14, marginBottom:20 }}>
                <div>
                  ...
                  <div style={{ fontFamily:"sans-serif", display:"flex", flexWrap:"wrap", alignItems:"center", gap:8, fontSize:13, color:"#5a7888" }}>
                    ...
                    <a href={`https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=${encodeURIComponent(meta.cik||ticker)}&type=10-K`} target="_blank" rel="noopener noreferrer" style={{ fontSize:12, borderBottom:"1px solid rgba(212,175,55,.3)", textDecoration:"none" }}>
                      Verify on SEC EDGAR ↗
                    </a>
                  </div>
                </div>
                <button onClick={reset} style={{ background:"transparent", border:"1px solid rgba(212,175,55,.22)", color:"#8a7540", padding:"8px 16px", borderRadius:9, cursor:"pointer", fontFamily:"sans-serif", fontSize:12, fontWeight:600 }}>
                  ← New search
                </button>
              </div>
            ...
            <div style={{ marginTop:32, padding:"16px 20px", background:"rgba(255,255,255,.015)", borderRadius:12, fontFamily:"sans-serif", fontSize:11, color:"#1e3040", lineHeight:1.7, textAlign:"center" }}>
              ⚖️ For informational purposes only · Not financial advice · Based on SEC 10-K filings ·{" "}
